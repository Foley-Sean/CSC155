...
//	shadowFactor = lookup(0.0, 0.0);
	vec3 R = reflect(V,N);
	vec4 textureColor = texture(t, R);
	vec4 shadowColor = textureColor * globalAmbient * light.ambient
					+ textureColor * light.ambient;

					//+ mvLight.ambient * material.ambient;

		vec4 lightedColor = textureColor * light.diffuse * max(dot(L,N),0.0)
